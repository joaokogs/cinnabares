...
function PokeIcon({ option, kind, className }: { option?: PokeOption; kind: Props["kind"]; className?: string }) {
  ...
  return <Image src={option.iconUrl} alt="" width={24} height={24} className={cn("object-contain", className)} onError={() => setFailed(true)} unoptimized />
}
...
export function PokeAutocomplete({ value, onChange, options, kind, placeholder, id, required = false }: Props) {
  ...
  const filteredOptions = options
    .filter((option) => option.name.includes(value.trim().toLowerCase()))
    .slice(0, 8)
  ...
  return (
    <div className="relative">
      <PokeIcon option={selectedOption} kind={kind} className="pointer-events-none absolute top-1/2 left-2 z-10 size-5 -translate-y-1/2" />
      <input
        id={id}
        required={required}
        value={value}
        ...
        className="h-9 w-full rounded-lg border border-input bg-background/70 pr-7 pl-9 text-sm outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30"
      />
          ...
          {filteredOptions.length > 0 ? <div id={`${id}-options`} role="listbox">
            {filteredOptions.map((option, index) => (
              <button
                ...
                className={cn("flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-xs hover:bg-muted", index === highlighted && "bg-muted")}
              >
                <PokeIcon option={option} kind={kind} className="size-6 shrink-0" />
