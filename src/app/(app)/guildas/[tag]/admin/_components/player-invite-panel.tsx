...
export function PlayerInvitePanel({ guildId }: { guildId: string }) {
  ...
  useLayoutEffect(() => {
    ...
    const updatePlacement = () => {
      ...
      const maxHeight = Math.max(96, Math.min(preferredHeight, availableSpace))
  ...
  const suggestionDropdown =
    !selectedPlayer && query.trim().length >= 2 && dropdownPosition && typeof document !== "undefined"
      ? createPortal(
          <div
            className="z-[100] overflow-y-auto rounded-lg border border-border bg-popover shadow-xl"
            style={{
              position: "fixed",
          ...
          >
            {isSearching ? (
              <p className="flex items-center gap-2 px-3 py-3 text-xs text-muted-foreground">
                <LoaderCircle className="size-3.5 animate-spin" /> Pesquisando...
              </p>
            ) : players.length > 0 ? (
              <div role="listbox" aria-label="Players encontrados">
                {players.map((player) => (
                  <button
                    key={player.id}
                    type="button"
                  ...
                  >
                    <div className="grid size-8 shrink-0 place-items-center overflow-hidden rounded-full bg-accent/15 text-xs font-bold text-accent">
                      {player.avatarUrl ? (
                        <Image src={player.avatarUrl} alt="" width={32} height={32} unoptimized className="size-full object-cover" />
                      ) : (
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="block truncate text-xs text-muted-foreground">@{player.username ?? "player"}</span>
                    </span>
                  </button>
                ))}
              </div>
            ) : (
              <p className="px-3 py-3 text-xs text-muted-foreground">Nenhum player encontrado.</p>
            )}
          </div>,
          document.body,
        )
  ...
  return (
    <>
      <Card className="border-border/70 bg-card/90">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <UserPlus className="size-5 text-accent" aria-hidden="true" /> Convidar player
          </CardTitle>
          <CardDescription>Pesquise pelo nome ou username e envie um convite individual.</CardDescription>
        </CardHeader>
          ...
          <form className="space-y-3" onSubmit={(event) => void handleInvite(event)}>
            <div className="relative">
              ...
              <input
                ...
                aria-label="Pesquisar player para convidar"
                autoComplete="off"
                className="h-10 w-full rounded-lg border border-input bg-background/70 pl-9 pr-3 text-sm outline-none transition focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30"
              />
            </div>
            {selectedPlayer ? (
              <p className="text-xs text-muted-foreground">
                Selecionado: <span className="font-semibold text-foreground">{selectedPlayer.name}</span> @{selectedPlayer.username ?? "player"}
              </p>
            ) : null}
            <Button type="submit" disabled={isInviting}>
              {isInviting ? <LoaderCircle className="animate-spin" aria-hidden="true" /> : <UserPlus aria-hidden="true" />}
              {isInviting ? "Enviando..." : "Enviar convite"}
            </Button>
          </form>
          {success ? (
            <p className="rounded-lg border border-green-500/30 bg-green-500/10 px-3 py-2 text-sm text-green-400" role="status">
              {success}
            </p>
          ) : null}
        </CardContent>
      ...
      <Dialog open={Boolean(error)} onOpenChange={(open) => { if (!open) setError(null) }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Não foi possível enviar o convite</DialogTitle>
            <DialogDescription>{error}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button type="button" onClick={() => setError(null)}>Entendi</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
