...
import { getMentionOptions, MentionTextarea, MoveHoverCard } from "@/components/ui/mention-textarea"
...
import { getHiddenPowerType, HIDDEN_POWER_MOVES, type NamedOption, type PokeOption } from "@/hooks/use-pokeapi-data"
...
function getMoveType(info: MoveInfo | null, name: string) {
  return getHiddenPowerType(name) ?? info?.type ?? "normal"
}
...
function useMoveInfo(name: string) {
  const [state, setState] = useState<{ name: string; data: MoveInfo | null }>(() => ({ name, data: moveInfoCache.get(name) ?? null }))
  useEffect(() => {
    if (!name || moveInfoCache.has(name)) return
    let active = true
    const request = loadMoveInfo(name)
    request.then((nextData) => { if (active && nextData) setState({ name, data: nextData }) })
    return () => { active = false }
  }, [name])
  return state.name === name ? state.data : moveInfoCache.get(name) ?? null
}
...
function StatEditor({ build, data, onChange }: { build: BuildDraft; data: NonNullable<ReturnType<typeof usePokemonBuildData>["data"]>; onChange: (build: BuildDraft) => void }) {
  ...
  function statMax(kind: "ivs" | "evs", stat: BuildStat) {
    if (kind === "ivs") return 31
    return Math.min(252, build.evs[stat] + Math.max(0, remainingEvs))
  }
  function updateStat(kind: "ivs" | "evs", stat: BuildStat, rawValue: string) {
    const max = statMax(kind, stat)
...
function MoveSlot({ name, onClick, onClear }: { name: string; onClick: () => void; onClear: () => void }) {
  ...
  return <div className="group relative flex min-h-11 items-center gap-2 border border-border bg-muted/60 px-3 py-2.5" style={{ backgroundColor: `${color}15`, borderLeftColor: color, borderLeftWidth: 4 }}><button type="button" onClick={onClick} className="flex min-w-0 flex-1 text-left">{name ? <MoveHoverCard name={info?.name ?? name} className="flex min-w-0 flex-1 items-center gap-2.5"><MoveTypeIcon type={type} /><span className="flex-1 truncate text-xs font-semibold">{titleCase(info?.name ?? name)}</span><MoveMeta info={info} color={color} /></MoveHoverCard> : <span className="flex min-w-0 flex-1 items-center gap-2.5"><MoveTypeIcon type={type} /><span className="flex-1 truncate text-xs font-semibold">Move</span><MoveMeta info={info} color={color} /></span>}</button>{name ? <button type="button" onClick={onClear} className="absolute right-1 hidden size-5 place-items-center text-muted-foreground hover:text-destructive group-hover:grid" aria-label="Remover move"><X className="size-3" /></button> : null}</div>
}
