...
export default async function TournamentPage({ params }: PageProps) {
  ...
  return <main className="relative min-h-screen overflow-hidden bg-background px-4 py-10 sm:px-6 lg:py-14"><div className="pointer-events-none absolute inset-0 bg-grid opacity-30" aria-hidden="true" /><div className="relative z-10 mx-auto w-full max-w-6xl space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
     ...
     {(registration?.status === "approved" || isAdmin) && existingBracket ? <Card className="border-accent/30 bg-card/90"><CardHeader><div className="flex items-center gap-2"><Swords className="size-5 text-accent" aria-hidden="true" /><CardTitle>Chave do torneio</CardTitle></div><CardDescription>{currentTournament.status === "finished" ? "O torneio já foi finalizado. A chave reflete o resultado final." : "A chave já foi gerada e está ativa."}</CardDescription></CardHeader><CardContent><Button asChild size="sm"><Link href={`/torneios/${id}/bracket`}>Ver chave completa <ExternalLink className="ml-1 size-3.5" aria-hidden="true" /></Link></Button></CardContent></Card> : null}
  </div></main>
}
